...
public class PerfilDAO {
    public ArrayList<Perfil> listaPerfiles(){
        ...
        String url = props.getProperty("ws");
            SOAPMessage soapResponse = soapConnection.call(createSOAPRequestListaPerfiles(), url);

 
            // Recibe la respuesta SOAP y la procesa.
            arrperfiles = getSOAPResponseListaPerfiles(soapResponse);
    ...
    private SOAPMessage createSOAPRequestListaPerfiles() throws Exception {
    ...
    private ArrayList<Perfil> getSOAPResponseListaPerfiles(SOAPMessage soapResponse) throws Exception {
        ArrayList<Perfil> arrper = new ArrayList<Perfil>();
...
//                XPathConstants.STRING));
        NodeList nodeList = (NodeList) xpath.compile("//TipoPerfil").evaluate(XMLDoc, XPathConstants.NODESET);
...
//                usu.setId_tipo_perfil(Integer.parseInt(eElement.getElementsByTagName("ID_TIPO_PERFIL").item(0).getTextContent()));
                per.setId_perfil(Integer.parseInt(eElement.getElementsByTagName("ID_PERFIL").item(0).getTextContent()));
                per.setNombre(eElement.getElementsByTagName("NOMBRE").item(0).getTextContent());
                arrper.add(per);
            }
